...
namespace SoprodiApp.acceso
{
    public static class ReporteDB
    {
        ...
        internal static string eliminar_firma(string usuario_, string periodo, string firma)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["default"].ToString()))
            {
                conn.Open();
                string sql = @"delete from [dbo].[comisionFirmas]  where cod_usuario = '" + usuario_.Trim() + "' and codperiodo = '" + periodo + "';" +
                   " insert into comisionFirmas values ('" + periodo + "', '" + usuario_ + "', " + firma + ") ";
