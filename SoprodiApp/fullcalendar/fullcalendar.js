...
function flexibleCompare(a, b) {
	...
	if ($.type(a) === 'string' || $.type(b) === 'string') {
	    ...
	    //alert ("normal " + String(a).localeCompare(String(b)));
	    var aux = COMPARE_PROPIO(String(a).substring(0, String(a).indexOf('-')), String(b).substring(0, String(b).indexOf('-')));
	    //alert("aux " + aux);
	    return  aux;
	}
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			el = $("<div id='intervls' class='fc-toolbar'/>")
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($('<h2 id=\'aca\'>&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
